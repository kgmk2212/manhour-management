...
> ⚠️ **この文書は 2026-06-14 時点の手書き記録であり、現状と一致しない。**
> 例: 「js/ 19ファイル」と書かれているが実際は 34 ファイル（合計 37,069 行）、`ui.js` は
> 2,279 行ではなく 4,316 行、`actual-timeline.js` / `schedule.js` / `merge-core.js` などは記載自体が無い。
> 冒頭のパスも移行前の Mac 環境のもの。
>
> **ファイル一覧・行数・関数の在り処は `docs/CODEMAP.md`（自動生成・CI で鮮度保証）を参照すること。**
> 本文書は当時の設計意図を振り返るための履歴として残している。
