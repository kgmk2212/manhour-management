...
export function renderProgressSummaryCards(versionFilter, conditionVersions = getVersionsInCurrentReportCondition()) {
    ...
    container.innerHTML = `
        <div class="stat-card theme-bg theme-${currentThemeColor}">
            ...
            <div style="font-size: calc(16px * var(--ui-scale)); color: rgba(255,255,255,0.8); margin-top: 5px;">
        ...
        <div class="stat-card theme-bg theme-${currentThemeColor}">
            ...
            <div style="display: flex; gap: 8px; flex-wrap: wrap; margin-top: 8px;">
                <span style="background: rgba(255,255,255,0.2); padding: 3px 8px; border-radius: 10px; font-size: calc(15.5px * var(--ui-scale));">
            ...
            html += `
                <tr style="border-left: 4px solid ${borderColor};">
                    ...
                    <td style="padding: 10px; text-align: right; border-bottom: 1px solid #eee; font-weight: 600;">
                        ...
                        ${progress.variance !== 0
                    ? `<span style="font-size: calc(15.5px * var(--ui-scale)); color: ${progress.variance > 0 ? '#e74c3c' : '#27ae60'};">
...
export function generateProgressBar(version, task, process) {
    ...
    if (progressBarStyle === 'bottom') {
        ...
        const percentageHtml = showProgressPercentageSetting
            ? `<div style="font-size: calc(15.5px * var(--ui-scale)); color: #888; margin-top: 2px; text-align: center;">${displayRate}%</div>`
            : '';
...
function renderPhase1AccuracyAnalysis(filteredEstimates, filteredActuals) {
    ...
    html += `<h3 onclick="togglePhaseCollapse('phase1')" style="margin: 0; color: #495057; font-size: calc(19px * var(--ui-scale)); cursor: pointer; display: flex; align-items: center; user-select: none;">`;
    html += `<span id="phase1-arrow" style="margin-right: 10px; font-size: calc(16px * var(--ui-scale));">${phaseCollapsed.phase1 ? '▶' : '▼'}</span>`;
...
function renderProcessAccuracy(filteredEstimates, filteredActuals) {
    ...
    html += '<h4 style="margin: 0 0 10px 0; color: #495057; font-size: calc(17px * var(--ui-scale));">工程別見積精度</h4>';
    ...
    sortedProcesses.forEach(proc => {
        ...
        html += `<div style="font-size: calc(22px * var(--ui-scale)); font-weight: bold; color: ${isOverrun ? '#dc3545' : isGood ? '#28a745' : '#ffc107'};">${accuracy}%</div>`;
        html += `<div style="font-size: calc(16px * var(--ui-scale)); color: #6c757d;">${data.estimate.toFixed(1)}h → ${data.actual.toFixed(1)}h</div>`;
        html += '</div>';
    });
...
function renderAnomalyDetection(filteredEstimates, filteredActuals) {
    ...
    html += '<h4 style="margin: 0 0 10px 0; color: #721c24; font-size: calc(17px * var(--ui-scale));">異常値検出（50%以上超過）</h4>';
    html += `<div style="color: #856404; font-size: calc(16px * var(--ui-scale)); margin-bottom: 10px;">検出数: ${anomalies.length}件</div>`;
    html += '<div style="max-height: 200px; overflow-y: auto;">';

    anomalies.slice(0, 10).forEach(anomaly => {
        html += '<div style="background: #ffffff; padding: 8px; border-radius: 4px; margin-bottom: 5px; font-size: calc(15.5px * var(--ui-scale)); border: 1px solid #f5c6cb;">';
...
function renderWarningTasks(filteredEstimates, filteredActuals) {
    ...
    html += '<h4 style="margin: 0 0 10px 0; color: #495057; font-size: calc(17px * var(--ui-scale));">要注意タスク</h4>';
    ...
    warnings.slice(0, 5).forEach((warning, idx) => {
        ...
        html += `<div style="font-size: calc(15.5px * var(--ui-scale)); color: #495057;">見積: ${warning.estimate.toFixed(1)}h → 実績: ${warning.actual.toFixed(1)}h <span style="color: #dc3545; font-weight: bold;">(+${warning.overrun.toFixed(0)}%)</span></div>`;
        html += `<div style="font-size: calc(15.5px * var(--ui-scale)); color: #6c757d;">工程数: ${warning.processCount}</div>`;
        html += '</div>';
    });
...
function renderPhase2VisualAnalysis(filteredEstimates, filteredActuals, selectedMonth) {
    ...
    let html = '';
    html += `<div style="background: #f8f9fa; padding: 20px; border-radius: 12px; margin: 20px 0; border: 1px solid #dee2e6;">`;
    html += `<h3 onclick="togglePhaseCollapse('phase2')" style="margin: 0; color: #495057; font-size: calc(19px * var(--ui-scale)); cursor: pointer; display: flex; align-items: center; user-select: none;">`;
    html += `<span id="phase2-arrow" style="margin-right: 10px; font-size: calc(16px * var(--ui-scale));">${phaseCollapsed.phase2 ? '▶' : '▼'}</span>`;
...
function renderProcessBarChart(filteredEstimates, filteredActuals) {
    ...
    // バーが短くラベルが収まらない場合はラベルをバーの右外側に表示する。
    const REVIEW_STRIPE = 'repeating-linear-gradient(45deg, rgba(255,255,255,0.45) 0 4px, transparent 4px 8px)';
    const LABEL_FONT = `font-size: calc(15.5px * var(--ui-scale)); font-weight: 600;`;
    const renderBar = (total, review, color) => {
        const widthPct = maxHours > 0 ? (total / maxHours * 100) : 0;
        const mainPct = maxHours > 0 ? ((total - review) / maxHours * 100) : 0;
        const reviewPct = maxHours > 0 ? (review / maxHours * 100) : 0;
        const labelInside = widthPct > 12;

        let h = `<div style="height: 100%; width: ${widthPct.toFixed(1)}%; border-radius: 4px; overflow: hidden; display: flex; position: relative;">`;
        h += `<div style="background: ${color}; height: 100%; width: ${widthPct > 0 ? (mainPct / widthPct * 100).toFixed(1) : 0}%;"></div>`;
        if (review > 0) {
            h += `<div style="background: ${color}; background-image: ${REVIEW_STRIPE}; height: 100%; width: ${widthPct > 0 ? (reviewPct / widthPct * 100).toFixed(1) : 0}%;" title="レビュー ${review.toFixed(1)}h"></div>`;
        }
        if (labelInside) {
            h += `<span style="position: absolute; right: 5px; top: 0; height: 100%; display: flex; align-items: center; ${LABEL_FONT} color: white;">${total.toFixed(1)}h</span>`;
        }
        h += '</div>';
        if (!labelInside) {
            h += `<span style="position: absolute; left: calc(${widthPct.toFixed(1)}% + 6px); top: 0; height: 100%; display: flex; align-items: center; ${LABEL_FONT} color: #495057;">${total.toFixed(1)}h</span>`;
        }
        return h;
    };
    ...
    sortedProcesses.forEach(proc => {
        ...
        html += renderBar(data.estimate, data.estimateReview, '#4dabf7');
        html += '</div>';
        html += '<div style="text-align: right; font-size: calc(15.5px * var(--ui-scale)); color: #6c757d;">実績</div>';
        html += `<div style="background: #e9ecef; border-radius: 4px; height: 20px; position: relative;">`;
        html += renderBar(data.actual, data.actualReview, actColor);
        html += '</div>';
        html += '</div></div>';
    });
...
function renderMonthlyTrend(filteredEstimates, filteredActuals) {
    ...
    let html = '<div style="background: #ffffff; padding: 15px; border-radius: 8px; border: 1px solid #e9ecef;">';
    html += '<h4 style="margin: 0 0 10px 0; color: #495057; font-size: calc(17px * var(--ui-scale));">月別推移</h4>';
    ...
    sortedMonths.slice(-6).forEach(month => {
        ...
        html += '<span style="font-size: calc(15.5px * var(--ui-scale)); color: #6c757d; min-width: 40px;">見積</span>';
        ...
        html += '</div>';
        html += `<span style="font-size: calc(15.5px * var(--ui-scale)); color: #495057; min-width: 50px; text-align: right;">${data.estimate.toFixed(1)}h</span>`;
        html += '</div>';
        html += '<div style="display: flex; align-items: center; gap: 5px;">';
        html += '<span style="font-size: calc(15.5px * var(--ui-scale)); color: #6c757d; min-width: 40px;">実績</span>';
        html += `<div style="flex: 1; background: #e9ecef; border-radius: 4px; height: 20px;">`;
        html += `<div style="background: ${data.actual > data.estimate ? '#dc3545' : '#28a745'}; height: 100%; width: ${actWidth}%; border-radius: 4px;"></div>`;
        html += '</div>';
        html += `<span style="font-size: calc(15.5px * var(--ui-scale)); color: #495057; min-width: 50px; text-align: right;">${data.actual.toFixed(1)}h</span>`;
        html += '</div>';
        html += '</div>';
        html += `<div style="min-width: 60px; text-align: right; font-size: calc(15.5px * var(--ui-scale)); color: ${diff > 0 ? '#dc3545' : '#28a745'}; font-weight: 600; display: flex; align-items: center; justify-content: flex-end;">${diff > 0 ? '+' : ''}${diff.toFixed(1)}h</div>`;
        html += '</div></div>';
    });
...
function renderPhase3MemberAnalysis(filteredEstimates, filteredActuals, workingDaysPerMonth, capacity) {
    ...
    html += `<div style="background: #f8f9fa; padding: 20px; border-radius: 12px; margin: 20px 0; border: 1px solid #dee2e6;">`;
    html += `<h3 onclick="togglePhaseCollapse('phase3')" style="margin: 0; color: #495057; font-size: calc(19px * var(--ui-scale)); cursor: pointer; display: flex; align-items: center; user-select: none;">`;
    html += `<span id="phase3-arrow" style="margin-right: 10px; font-size: calc(16px * var(--ui-scale));">${phaseCollapsed.phase3 ? '▶' : '▼'}</span>`;
...
function renderMemberPerformance(filteredEstimates, filteredActuals, workingDaysPerMonth) {
    ...
    let html = '<div style="background: #ffffff; padding: 15px; border-radius: 8px; margin-bottom: 15px; border: 1px solid #e9ecef;">';
    html += '<h4 style="margin: 0 0 10px 0; color: #495057; font-size: calc(17px * var(--ui-scale));">担当者別パフォーマンス</h4>';

    // グラフセクション
    html += '<div style="margin-bottom: 20px;">';
    html += '<h5 style="margin: 0 0 10px 0; color: #495057; font-size: calc(16px * var(--ui-scale)); font-weight: 600;">見積 vs 実績 比較</h5>';
    ...
    members.forEach(member => {
        ...
        html += `<div style="font-size: calc(15.5px * var(--ui-scale)); color: #495057; margin-bottom: 2px;">見積: ${data.estimate.toFixed(1)}h</div>`;
        html += `<div style="font-size: calc(15.5px * var(--ui-scale)); color: #6c757d; margin-left: 10px; margin-bottom: 6px;">${estManDays}人日 / ${estManMonths}人月</div>`;
        html += `<div style="font-size: calc(15.5px * var(--ui-scale)); color: #495057; margin-bottom: 2px;">実績: ${data.actual.toFixed(1)}h</div>`;
        html += `<div style="font-size: calc(15.5px * var(--ui-scale)); color: #6c757d; margin-left: 10px; margin-bottom: 6px;">${actManDays}人日 / ${actManMonths}人月</div>`;
        html += `<div style="font-size: calc(15.5px * var(--ui-scale)); color: #495057;">精度: <span style="font-weight: 600;">${accuracy}%</span></div>`;
        html += `<div style="font-size: calc(15.5px * var(--ui-scale)); color: #495057;">差分: <span style="color: ${diff > 0 ? '#dc3545' : '#28a745'};">${diff > 0 ? '+' : ''}${diff.toFixed(1)}h</span></div>`;
        html += `<div style="font-size: calc(15.5px * var(--ui-scale)); color: #6c757d; margin-top: 5px;">担当タスク: ${taskCount}件</div>`;
        html += '</div>';
    });
    ...
    html += '<h5 style="margin: 0 0 10px 0; color: #495057; font-size: calc(16px * var(--ui-scale)); font-weight: 600;">担当者別 工数内訳</h5>';
...
function renderInsights(filteredEstimates, filteredActuals, capacity) {
    ...
    let html = '<div style="background: #ffffff; padding: 15px; border-radius: 8px; border: 1px solid #e9ecef;">';
    html += '<h4 style="margin: 0 0 10px 0; color: #495057; font-size: calc(17px * var(--ui-scale));">インサイト</h4>';
    ...
    versions.forEach(version => {
        ...
        Object.values(versionGroups[version]).forEach(taskGroup => {
            ...
            sortedProcesses.forEach((proc, index) => {
                ...
                tableBody += `<td style="${progressCellStyle}">${progressBarHtml || '<span style="color: #ccc; font-size: calc(15.5px * var(--ui-scale));">-</span>'}</td>`;
                if (index === 0) {
        ...
        if (tableBody) {
            ...
            html += `<div style="margin-bottom: 30px;">`;
            html += `<h3 class="version-header theme-bg theme-${currentThemeColor}" style="color: white; padding: 12px 20px; border-radius: 8px; margin: 0 0 15px 0; font-size: calc(19px * var(--ui-scale));">${escapeHtml(version)}</h3>`;
...
export function evaluateMatrixCellColor({ estHours = 0, actHours = 0, remainingHours = 0 } = {}) {
    ...
    if (act > 0) return 'over';  // 見積なしで実績あり → 赤
 ...
 * マトリクスセルの進捗率(%)を求める。
 *
 * 見込残存が工程全体の値なので、分子の実績も工程全体（全期間）の値を渡すこと。
 * 月で切ると前月までの実績が分子から消え、実績のない月で 0% に見えてしまう。
 *
 * @param {{actHours: number, remainingHours: number}} params - 全期間の実績と工程全体の見込残存
 * @returns {number} 進捗率(%)
 */
export function calcMatrixProgressRate({ actHours = 0, remainingHours = 0 } = {}) {
    const act = Number(actHours) || 0;
    const remaining = Math.max(0, Number(remainingHours) || 0);
    const eac = act + remaining;

    return eac > 0 ? (act / eac) * 100 : 0;
}
...
export function renderReportMatrix(filteredActuals, filteredEstimates, selectedMonth) {
    ...
    // 見込残存は工程全体の値なので、進捗率は月フィルタ前の全期間データで求める（B-039①）
    const fullRangeTotals = buildFullRangeTotals(estimates, actuals, isOtherWork);
    // 月別表示の文字色は「その月の見積 vs その月の実績」で判定する（残存は月に割り振れない）
    const isMonthFiltered = !!selectedMonth && selectedMonth !== 'all';
    ...
    versions.forEach(version => {
            ...
            if (totalEst > 0 || totalAct > 0) {
                ...
                if (isOtherWorkVersion) {
                    ...
                    contentHtml += `<td style="text-align: center;">
                        ...
                        <div class="total-manpower">
                            <div style="font-size: calc(15.5px * var(--ui-scale)); color: #666;">${totalEstDays}/${totalActDays}人日</div>
                            <div style="font-size: calc(15.5px * var(--ui-scale)); color: #666;">${totalEstMonths}/${totalActMonths}人月</div>
                        </div>
                    </td>`;
                    contentHtml += '</tr>';
                    return;
                ...
                taskCells.forEach(({ proc, est, act }) => {
                    if (est.hours > 0 || act.hours > 0) {
                        ...
                        // デバッグ: 問題調査用ログ
                        if (debugModeEnabled) {
                            const colorBasis = isMonthFiltered
                                ? `月別(${act.hours}/${est.hours})`
                                : `全期間(${fullRange.act}+${cellRemainingHours}/${fullRange.est})`;
                            const progressRate = calcMatrixProgressRate({ actHours: fullRange.act, remainingHours: cellRemainingHours });
                            console.log(`[Matrix Debug] ${version}/${taskGroup.task}/${proc}: est=${est.hours}(全期間${fullRange.est}), act=${act.hours}(全期間${fullRange.act}), remaining=${cellRemainingHours}, remainingDataCount=${remainingData.length}, usedFallback=${usedFallback}, color=${colorBasis}, progress=${progressRate.toFixed(1)}%`);
                        }
                        ...
                        const cellInner = renderCellOptionA(version, taskGroup.task, proc, est, act, bgColorMode, workingDaysPerMonth, cellRemainingHours, { fullRange, isMonthFiltered });
                ...
                const totalCellInner = renderCellOptionA(
                    ...
                    workingDaysPerMonth,
                    totalRemainingHours,
                    { fullRange: totalFullRange, isMonthFiltered }
                );
        ...
        if (contentHtml) {
            ...
            html += `<td style="text-align: center; background: ${vBg};">
                ...
                <div class="total-manpower">
                    <div style="font-size: calc(15.5px * var(--ui-scale)); color: #666;">${versionTotalEstDays.toFixed(1)}/${versionTotalActDays.toFixed(1)}人日</div>
                    <div style="font-size: calc(15.5px * var(--ui-scale)); color: #666;">${versionTotalEstMonths.toFixed(2)}/${versionTotalActMonths.toFixed(2)}人月</div>
                </div>
            </td></tr></table></div>`;
...
function renderCellOptionA(version, task, process, est, act, bgColorMode, workingDaysPerMonth = 20, remainingHours = null, evalBasis = null) {
    const diff = act.hours - est.hours;
    ...
    // 進捗率は見込残存と粒度を揃えるため工程全体（全期間）の実績で求める（B-039①）
    const fullRange = evalBasis && evalBasis.fullRange ? evalBasis.fullRange : null;
    const fullActHours = fullRange ? fullRange.act : act.hours;
    const fullEstHours = fullRange ? fullRange.est : est.hours;

    // 文字色は月別表示なら「その月の見積 vs その月の実績」、全期間表示なら
    // 「見積 vs 予測総工数(実績+見込残存)」。見込残存は月に割り振れないので月別では使わない。
    const isMonthFiltered = !!(evalBasis && evalBasis.isMonthFiltered);
    const actColorClass = isMonthFiltered
        ? evaluateMatrixCellColor({ estHours: est.hours, actHours: act.hours })
        : evaluateMatrixCellColor({ estHours: fullEstHours, actHours: fullActHours, remainingHours: remaining });
    ...
    if (showProgressBarsSetting && process !== 'total') {
        ...
        const actualHours = fullActHours || 0;
        ...
        // 進捗率を計算（実績 / (実績 + 見込み残存) × 100）
        const progressRate = calcMatrixProgressRate({ actHours: actualHours, remainingHours: remaining });

        // 進捗率が0%の場合はバーを表示しない（罫線と紛らわしいため）
        if ((fullEstHours > 0 || actualHours > 0) && progressRate > 0) {
            const barColor = getProgressColor(progressRate);
    ...
    if (process === 'total') {
        ...
        return `
            <div style="text-align: center;">
                ...
                <div class="total-manpower">
                    <div style="font-size: calc(15.5px * var(--ui-scale)); color: #666;">${estDays.toFixed(1)}/${actDays.toFixed(1)}人日</div>
                    <div style="font-size: calc(15.5px * var(--ui-scale)); color: #666;">${estMonths.toFixed(2)}/${actMonths.toFixed(2)}人月</div>
                </div>
            </div>
        `;
    }
    ...
    const manDaysHtml = (est.hours > 0 || act.hours > 0)
        ? `<div class="cell-mandays" style="font-size: calc(15.5px * var(--ui-scale)); color: #888;">${estDays.toFixed(1)}/${actDays.toFixed(1)}人日</div>`
        : '';
    ...
    return `
        <div style="${wrapperStyle}">
            ...
            ${memberDisplay ? (isMultiMember
                ? `<div style="font-size: calc(15.5px * var(--ui-scale)); color: #1976d2; white-space: nowrap; cursor: pointer; text-decoration: underline;" onclick="event.stopPropagation(); openProcessBreakdown('${escapeForHandler(version)}', '${escapeForHandler(task)}', '${escapeForHandler(process)}')" title="クリックで担当者別内訳を表示">(${escapeHtml(memberDisplay)})</div>`
                : `<div style="font-size: calc(15.5px * var(--ui-scale)); color: #666; white-space: nowrap;">(${escapeHtml(memberDisplay)})</div>`) : ''}
            ${progressBarHtml}
        </div>
    `;
}
