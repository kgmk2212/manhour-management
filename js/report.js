...
export function renderProgressSummaryCards(versionFilter) {
    ...
    container.innerHTML = `
        <div class="stat-card theme-bg theme-${currentThemeColor}">
            ...
            <div style="font-size: calc(14px * var(--ui-scale)); color: rgba(255,255,255,0.8); margin-top: 5px;">
            ...
            html += `
                <tr style="border-left: 4px solid ${borderColor};">
                    ...
                    <td style="padding: 10px; text-align: right; border-bottom: 1px solid #eee; font-weight: 600;">
                        ...
                        ${progress.variance !== 0
                    ? `<span style="font-size: calc(13px * var(--ui-scale)); color: ${progress.variance > 0 ? '#e74c3c' : '#27ae60'};">
...
export function generateProgressBar(version, task, process) {
    ...
    if (progressBarStyle === 'bottom') {
        ...
        const percentageHtml = showProgressPercentageSetting
            ? `<div style="font-size: calc(9px * var(--ui-scale)); color: #888; margin-top: 2px; text-align: center;">${displayRate}%</div>`
            : '';
...
function renderPhase1AccuracyAnalysis(filteredEstimates, filteredActuals) {
    ...
    html += `<span id="phase1-arrow" style="margin-right: 10px; font-size: calc(14px * var(--ui-scale));">${phaseCollapsed.phase1 ? '▶' : '▼'}</span>`;
...
function renderProcessAccuracy(filteredEstimates, filteredActuals) {
    ...
    sortedProcesses.forEach(proc => {
        ...
        html += `<div style="font-size: calc(14px * var(--ui-scale)); color: #6c757d;">${data.estimate.toFixed(1)}h → ${data.actual.toFixed(1)}h</div>`;
        html += '</div>';
    });
...
function renderAnomalyDetection(filteredEstimates, filteredActuals) {
    ...
    html += `<div style="color: #856404; font-size: calc(14px * var(--ui-scale)); margin-bottom: 10px;">検出数: ${anomalies.length}件</div>`;
    html += '<div style="max-height: 200px; overflow-y: auto;">';

    anomalies.slice(0, 10).forEach(anomaly => {
        html += '<div style="background: #ffffff; padding: 8px; border-radius: 4px; margin-bottom: 5px; font-size: calc(13px * var(--ui-scale)); border: 1px solid #f5c6cb;">';
...
function renderPhase2VisualAnalysis(filteredEstimates, filteredActuals, selectedMonth) {
    ...
    html += `<span id="phase2-arrow" style="margin-right: 10px; font-size: calc(14px * var(--ui-scale));">${phaseCollapsed.phase2 ? '▶' : '▼'}</span>`;
...
function renderPhase3MemberAnalysis(filteredEstimates, filteredActuals, workingDaysPerMonth) {
    ...
    html += `<span id="phase3-arrow" style="margin-right: 10px; font-size: calc(14px * var(--ui-scale));">${phaseCollapsed.phase3 ? '▶' : '▼'}</span>`;
...
function renderMemberPerformance(filteredEstimates, filteredActuals, workingDaysPerMonth) {
    ...
    html += '<h5 style="margin: 0 0 10px 0; color: #495057; font-size: calc(14px * var(--ui-scale)); font-weight: 600;">見積 vs 実績 比較</h5>';
    ...
    html += '<h5 style="margin: 0 0 10px 0; color: #495057; font-size: calc(14px * var(--ui-scale)); font-weight: 600;">担当者別 工数内訳</h5>';
...
function renderInsights(filteredEstimates, filteredActuals) {
    ...
    insights.forEach(insight => {
        ...
        html += `<div style="font-size: calc(13px * var(--ui-scale)); color: ${textColor};">${insight.message}</div>`;
        html += '</div>';
    });
            ...
            sortedProcesses.forEach((proc, index) => {
                ...
                if (index === 0) {
                    ...
                    if (taskGroup.task.includes('：')) {
                        ...
                        taskDisplayHtml = `${escapeHtml(parts[0])}<br><span style="font-size: calc(13px * var(--ui-scale)); font-weight: normal;">${escapeHtml(restPart)}</span>`;
                    }
                ...
                tableBody += `<td style="${progressCellStyle}">${progressBarHtml || '<span style="color: #ccc; font-size: calc(13px * var(--ui-scale));">-</span>'}</td>`;
                if (index === 0) {
    ...
    versions.forEach(version => {
                ...
                if (isOtherWorkVersion) {
                    ...
                    contentHtml += `<td style="text-align: center;">
                        ...
                        <div class="total-manpower">
                            <div style="font-size: calc(11px * var(--ui-scale)); color: #666;">${totalEstDays}/${totalActDays}人日</div>
                            <div style="font-size: calc(11px * var(--ui-scale)); color: #666;">${totalEstMonths}/${totalActMonths}人月</div>
                        </div>
                    </td>`;
                    contentHtml += '</tr>';
                    return;
        ...
        if (contentHtml) {
            ...
            html += `<td style="text-align: center; background: ${vBg};">
                ...
                <div class="total-manpower">
                    <div style="font-size: calc(11px * var(--ui-scale)); color: #666;">${versionTotalEstDays.toFixed(1)}/${versionTotalActDays.toFixed(1)}人日</div>
                    <div style="font-size: calc(11px * var(--ui-scale)); color: #666;">${versionTotalEstMonths.toFixed(2)}/${versionTotalActMonths.toFixed(2)}人月</div>
                </div>
            </td></tr></table></div>`;
...
function renderCellOptionA(version, task, process, est, act, bgColorMode, workingDaysPerMonth = 20, remainingHours = null) {
    ...
    if (process === 'total') {
        ...
        return `
            <div style="text-align: center;">
                ...
                <div class="total-manpower">
                    <div style="font-size: calc(11px * var(--ui-scale)); color: #666;">${estDays.toFixed(1)}/${actDays.toFixed(1)}人日</div>
                    <div style="font-size: calc(11px * var(--ui-scale)); color: #666;">${estMonths.toFixed(2)}/${actMonths.toFixed(2)}人月</div>
                </div>
            </div>
        `;
    }
    ...
    const manDaysHtml = (est.hours > 0 || act.hours > 0)
        ? `<div class="cell-mandays" style="font-size: calc(10px * var(--ui-scale)); color: #888;">${estDays.toFixed(1)}/${actDays.toFixed(1)}人日</div>`
        : '';
