...
export function renderProgressSummaryCards(versionFilter) {
    ...
    container.innerHTML = `
        <div class="stat-card theme-bg theme-${currentThemeColor}">
            ...
            <div style="font-size: calc(15px * var(--ui-scale)); color: rgba(255,255,255,0.8); margin-top: 5px;">
        ...
        <div class="stat-card theme-bg theme-${currentThemeColor}">
            ...
            <div style="display: flex; gap: 8px; flex-wrap: wrap; margin-top: 8px;">
                <span style="background: rgba(255,255,255,0.2); padding: 3px 8px; border-radius: 10px; font-size: calc(14.5px * var(--ui-scale));">
            ...
            html += `
                <tr style="border-left: 4px solid ${borderColor};">
                    ...
                    <td style="padding: 10px; text-align: right; border-bottom: 1px solid #eee; font-weight: 600;">
                        ...
                        ${progress.variance !== 0
                    ? `<span style="font-size: calc(14.5px * var(--ui-scale)); color: ${progress.variance > 0 ? '#e74c3c' : '#27ae60'};">
...
export function generateProgressBar(version, task, process) {
    ...
    if (progressBarStyle === 'bottom') {
        ...
        const percentageHtml = showProgressPercentageSetting
            ? `<div style="font-size: calc(14.5px * var(--ui-scale)); color: #888; margin-top: 2px; text-align: center;">${displayRate}%</div>`
            : '';
...
function renderPhase1AccuracyAnalysis(filteredEstimates, filteredActuals) {
    ...
    html += `<h3 onclick="togglePhaseCollapse('phase1')" style="margin: 0; color: #495057; font-size: calc(18px * var(--ui-scale)); cursor: pointer; display: flex; align-items: center; user-select: none;">`;
    html += `<span id="phase1-arrow" style="margin-right: 10px; font-size: calc(15px * var(--ui-scale));">${phaseCollapsed.phase1 ? '▶' : '▼'}</span>`;
...
function renderProcessAccuracy(filteredEstimates, filteredActuals) {
    ...
    html += '<h4 style="margin: 0 0 10px 0; color: #495057; font-size: calc(16px * var(--ui-scale));">工程別見積精度</h4>';
    ...
    sortedProcesses.forEach(proc => {
        ...
        html += `<div style="font-size: calc(15px * var(--ui-scale)); color: #6c757d;">${data.estimate.toFixed(1)}h → ${data.actual.toFixed(1)}h</div>`;
        html += '</div>';
    });
...
function renderAnomalyDetection(filteredEstimates, filteredActuals) {
    ...
    html += '<h4 style="margin: 0 0 10px 0; color: #721c24; font-size: calc(16px * var(--ui-scale));">異常値検出（50%以上超過）</h4>';
    html += `<div style="color: #856404; font-size: calc(15px * var(--ui-scale)); margin-bottom: 10px;">検出数: ${anomalies.length}件</div>`;
    html += '<div style="max-height: 200px; overflow-y: auto;">';

    anomalies.slice(0, 10).forEach(anomaly => {
        html += '<div style="background: #ffffff; padding: 8px; border-radius: 4px; margin-bottom: 5px; font-size: calc(14.5px * var(--ui-scale)); border: 1px solid #f5c6cb;">';
...
function renderWarningTasks(filteredEstimates, filteredActuals) {
    ...
    html += '<h4 style="margin: 0 0 10px 0; color: #495057; font-size: calc(16px * var(--ui-scale));">要注意タスク</h4>';
    ...
    warnings.slice(0, 5).forEach((warning, idx) => {
        ...
        html += `<div style="font-size: calc(14.5px * var(--ui-scale)); color: #495057;">見積: ${warning.estimate.toFixed(1)}h → 実績: ${warning.actual.toFixed(1)}h <span style="color: #dc3545; font-weight: bold;">(+${warning.overrun.toFixed(0)}%)</span></div>`;
        html += `<div style="font-size: calc(14.5px * var(--ui-scale)); color: #6c757d;">工程数: ${warning.processCount}</div>`;
        html += '</div>';
    });
...
function renderPhase2VisualAnalysis(filteredEstimates, filteredActuals, selectedMonth) {
    ...
    let html = '';
    html += `<div style="background: #f8f9fa; padding: 20px; border-radius: 12px; margin: 20px 0; border: 1px solid #dee2e6;">`;
    html += `<h3 onclick="togglePhaseCollapse('phase2')" style="margin: 0; color: #495057; font-size: calc(18px * var(--ui-scale)); cursor: pointer; display: flex; align-items: center; user-select: none;">`;
    html += `<span id="phase2-arrow" style="margin-right: 10px; font-size: calc(15px * var(--ui-scale));">${phaseCollapsed.phase2 ? '▶' : '▼'}</span>`;
...
function renderProcessBarChart(filteredEstimates, filteredActuals) {
    ...
    let html = '<div style="background: #ffffff; padding: 15px; border-radius: 8px; margin-bottom: 15px; border: 1px solid #e9ecef;">';
    html += '<h4 style="margin: 0 0 10px 0; color: #495057; font-size: calc(16px * var(--ui-scale));">工程別見積vs実績</h4>';
    ...
    sortedProcesses.forEach(proc => {
        ...
        html += '<div style="text-align: right; font-size: calc(14.5px * var(--ui-scale)); color: #6c757d;">見積</div>';
        ...
        html += `<span style="font-size: calc(14.5px * var(--ui-scale)); font-weight: 600; color: white;">${data.estimate.toFixed(1)}h</span>`;
        html += '</div></div>';
        html += '<div style="text-align: right; font-size: calc(14.5px * var(--ui-scale)); color: #6c757d;">実績</div>';
        ...
        html += `<span style="font-size: calc(14.5px * var(--ui-scale)); font-weight: 600; color: white;">${data.actual.toFixed(1)}h</span>`;
        html += '</div></div>';
        html += '</div></div>';
    });
...
function renderMonthlyTrend(filteredEstimates, filteredActuals) {
    ...
    let html = '<div style="background: #ffffff; padding: 15px; border-radius: 8px; border: 1px solid #e9ecef;">';
    html += '<h4 style="margin: 0 0 10px 0; color: #495057; font-size: calc(16px * var(--ui-scale));">月別推移</h4>';
    ...
    sortedMonths.slice(-6).forEach(month => {
        ...
        html += '<span style="font-size: calc(14.5px * var(--ui-scale)); color: #6c757d; min-width: 40px;">見積</span>';
        ...
        html += '</div>';
        html += `<span style="font-size: calc(14.5px * var(--ui-scale)); color: #495057; min-width: 50px; text-align: right;">${data.estimate.toFixed(1)}h</span>`;
        html += '</div>';
        html += '<div style="display: flex; align-items: center; gap: 5px;">';
        html += '<span style="font-size: calc(14.5px * var(--ui-scale)); color: #6c757d; min-width: 40px;">実績</span>';
        html += `<div style="flex: 1; background: #e9ecef; border-radius: 4px; height: 20px;">`;
        html += `<div style="background: ${data.actual > data.estimate ? '#dc3545' : '#28a745'}; height: 100%; width: ${actWidth}%; border-radius: 4px;"></div>`;
        html += '</div>';
        html += `<span style="font-size: calc(14.5px * var(--ui-scale)); color: #495057; min-width: 50px; text-align: right;">${data.actual.toFixed(1)}h</span>`;
        html += '</div>';
        html += '</div>';
        html += `<div style="min-width: 60px; text-align: right; font-size: calc(14.5px * var(--ui-scale)); color: ${diff > 0 ? '#dc3545' : '#28a745'}; font-weight: 600; display: flex; align-items: center; justify-content: flex-end;">${diff > 0 ? '+' : ''}${diff.toFixed(1)}h</div>`;
        html += '</div></div>';
    });
...
function renderPhase3MemberAnalysis(filteredEstimates, filteredActuals, workingDaysPerMonth) {
    ...
    html += `<div style="background: #f8f9fa; padding: 20px; border-radius: 12px; margin: 20px 0; border: 1px solid #dee2e6;">`;
    html += `<h3 onclick="togglePhaseCollapse('phase3')" style="margin: 0; color: #495057; font-size: calc(18px * var(--ui-scale)); cursor: pointer; display: flex; align-items: center; user-select: none;">`;
    html += `<span id="phase3-arrow" style="margin-right: 10px; font-size: calc(15px * var(--ui-scale));">${phaseCollapsed.phase3 ? '▶' : '▼'}</span>`;
...
function renderMemberPerformance(filteredEstimates, filteredActuals, workingDaysPerMonth) {
    ...
    let html = '<div style="background: #ffffff; padding: 15px; border-radius: 8px; margin-bottom: 15px; border: 1px solid #e9ecef;">';
    html += '<h4 style="margin: 0 0 10px 0; color: #495057; font-size: calc(16px * var(--ui-scale));">担当者別パフォーマンス</h4>';

    // グラフセクション
    html += '<div style="margin-bottom: 20px;">';
    html += '<h5 style="margin: 0 0 10px 0; color: #495057; font-size: calc(15px * var(--ui-scale)); font-weight: 600;">見積 vs 実績 比較</h5>';
    ...
    members.forEach(member => {
        ...
        html += `<div style="font-size: calc(14.5px * var(--ui-scale)); color: #495057; margin-bottom: 2px;">見積: ${data.estimate.toFixed(1)}h</div>`;
        html += `<div style="font-size: calc(14.5px * var(--ui-scale)); color: #6c757d; margin-left: 10px; margin-bottom: 6px;">${estManDays}人日 / ${estManMonths}人月</div>`;
        html += `<div style="font-size: calc(14.5px * var(--ui-scale)); color: #495057; margin-bottom: 2px;">実績: ${data.actual.toFixed(1)}h</div>`;
        html += `<div style="font-size: calc(14.5px * var(--ui-scale)); color: #6c757d; margin-left: 10px; margin-bottom: 6px;">${actManDays}人日 / ${actManMonths}人月</div>`;
        html += `<div style="font-size: calc(14.5px * var(--ui-scale)); color: #495057;">精度: <span style="font-weight: 600;">${accuracy}%</span></div>`;
        html += `<div style="font-size: calc(14.5px * var(--ui-scale)); color: #495057;">差分: <span style="color: ${diff > 0 ? '#dc3545' : '#28a745'};">${diff > 0 ? '+' : ''}${diff.toFixed(1)}h</span></div>`;
        html += `<div style="font-size: calc(14.5px * var(--ui-scale)); color: #6c757d; margin-top: 5px;">担当タスク: ${taskCount}件</div>`;
        html += '</div>';
    });
    ...
    html += '<h5 style="margin: 0 0 10px 0; color: #495057; font-size: calc(15px * var(--ui-scale)); font-weight: 600;">担当者別 工数内訳</h5>';
...
function renderInsights(filteredEstimates, filteredActuals) {
    ...
    let html = '<div style="background: #ffffff; padding: 15px; border-radius: 8px; border: 1px solid #e9ecef;">';
    html += '<h4 style="margin: 0 0 10px 0; color: #495057; font-size: calc(16px * var(--ui-scale));">インサイト</h4>';
    ...
    insights.forEach(insight => {
        ...
        html += `<div style="font-size: calc(14.5px * var(--ui-scale)); color: ${textColor};">${insight.message}</div>`;
        html += '</div>';
    });
...
export function renderReportGrouped(filteredActuals, filteredEstimates) {
    ...
    versions.forEach(version => {
            ...
            sortedProcesses.forEach((proc, index) => {
                ...
                if (index === 0) {
                    ...
                    if (taskGroup.task.includes('：')) {
                        ...
                        taskDisplayHtml = `${escapeHtml(parts[0])}<br><span style="font-size: calc(14.5px * var(--ui-scale)); font-weight: normal;">${escapeHtml(restPart)}</span>`;
                    }
                ...
                tableBody += `<td style="${progressCellStyle}">${progressBarHtml || '<span style="color: #ccc; font-size: calc(14.5px * var(--ui-scale));">-</span>'}</td>`;
                if (index === 0) {
        ...
        if (tableBody) {
            ...
            html += `<div style="margin-bottom: 30px;">`;
            html += `<h3 class="version-header theme-bg theme-${currentThemeColor}" style="color: white; padding: 12px 20px; border-radius: 8px; margin: 0 0 15px 0; font-size: calc(18px * var(--ui-scale));">${escapeHtml(version)}</h3>`;
    ...
    versions.forEach(version => {
                ...
                if (isOtherWorkVersion) {
                    ...
                    contentHtml += `<td style="text-align: center;">
                        ...
                        <div class="total-manpower">
                            <div style="font-size: calc(14.5px * var(--ui-scale)); color: #666;">${totalEstDays}/${totalActDays}人日</div>
                            <div style="font-size: calc(14.5px * var(--ui-scale)); color: #666;">${totalEstMonths}/${totalActMonths}人月</div>
                        </div>
                    </td>`;
                    contentHtml += '</tr>';
                    return;
        ...
        if (contentHtml) {
            ...
            html += `<td style="text-align: center; background: ${vBg};">
                ...
                <div class="total-manpower">
                    <div style="font-size: calc(14.5px * var(--ui-scale)); color: #666;">${versionTotalEstDays.toFixed(1)}/${versionTotalActDays.toFixed(1)}人日</div>
                    <div style="font-size: calc(14.5px * var(--ui-scale)); color: #666;">${versionTotalEstMonths.toFixed(2)}/${versionTotalActMonths.toFixed(2)}人月</div>
                </div>
            </td></tr></table></div>`;
...
function renderCellOptionA(version, task, process, est, act, bgColorMode, workingDaysPerMonth = 20, remainingHours = null) {
    ...
    if (process === 'total') {
        ...
        return `
            <div style="text-align: center;">
                ...
                <div class="total-manpower">
                    <div style="font-size: calc(14.5px * var(--ui-scale)); color: #666;">${estDays.toFixed(1)}/${actDays.toFixed(1)}人日</div>
                    <div style="font-size: calc(14.5px * var(--ui-scale)); color: #666;">${estMonths.toFixed(2)}/${actMonths.toFixed(2)}人月</div>
                </div>
            </div>
        `;
    }
    ...
    const manDaysHtml = (est.hours > 0 || act.hours > 0)
        ? `<div class="cell-mandays" style="font-size: calc(14.5px * var(--ui-scale)); color: #888;">${estDays.toFixed(1)}/${actDays.toFixed(1)}人日</div>`
        : '';
    ...
    return `
        <div style="${wrapperStyle}">
            ...
            ${memberDisplay ? (isMultiMember
                ? `<div style="font-size: calc(14.5px * var(--ui-scale)); color: #1976d2; white-space: nowrap; cursor: pointer; text-decoration: underline;" onclick="event.stopPropagation(); openProcessBreakdown('${escapeForHandler(version)}', '${escapeForHandler(task)}', '${escapeForHandler(process)}')" title="クリックで担当者別内訳を表示">(${escapeHtml(memberDisplay)})</div>`
                : `<div style="font-size: calc(14.5px * var(--ui-scale)); color: #666; white-space: nowrap;">(${escapeHtml(memberDisplay)})</div>`) : ''}
            ${progressBarHtml}
        </div>
    `;
}
