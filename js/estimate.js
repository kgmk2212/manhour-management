...
function updateEstimateStandardDisplay(totalHours, workingDays, headcount, monthFilter) {
    ...
    if (mode === 'subtext') {
        ...
        const diffLabel = diff >= 0
            ? `<span style="background: rgba(255,255,255,0.15); padding: 2px 10px; border-radius: 12px; font-size: calc(14px * var(--ui-scale)); font-weight: 600;">余裕 ${absDiff}h</span>`
            : `<span style="background: rgba(239,68,68,0.3); padding: 2px 10px; border-radius: 12px; font-size: calc(14px * var(--ui-scale)); font-weight: 600;">超過 ${absDiff}h</span>`;
        elB.innerHTML = `<div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 6px;">
            <span style="opacity: 0.85;">月標準工数: <strong style="font-size: calc(16px * var(--ui-scale));">${standardHours}h</strong> <span style="opacity: 0.7; font-size: calc(14px * var(--ui-scale));">（${workingDays}日×8h×${headcount}人）</span></span>
            ${diffLabel}
        </div>`;
    }
}
    ...
    sortedMembers.forEach(member => {
        ...
        memberHtml += `
            <div style="background: white; padding: 10px 15px; border-radius: 6px; border-left: 4px solid ${borderColor}; min-width: 150px;">
                <div style="font-size: calc(14px * var(--ui-scale)); color: #666; margin-bottom: 3px;">${escapeHtml(member)}</div>
                <div style="font-size: calc(18px * var(--ui-scale)); font-weight: 700; color: #333;">${hours.toFixed(1)}h</div>
                <div style="font-size: calc(14px * var(--ui-scale)); color: #666; font-weight: 500;">${days}人日 / ${months}人月</div>
            </div>
        `;
    });
    ...
    sortedVersionKeys.forEach(version => {
        ...
        if (isOtherWorkVersion) {
            ...
            Object.values(versionGroups[version]).forEach(taskGroup => {
                ...
                html += `<td style="text-align: right;">
                    ...
                    <div class="manpower-display" style="font-size: calc(14px * var(--ui-scale)); color: #666;">${formatNumber(days, 1)}人日 / ${formatNumber(months, 2)}人月</div>
                </td>`;
                html += '</tr>';
            });
        ...
        sortedTaskGroups.forEach(taskGroup => {
            ...
            if (taskWorkMonths.length === 0) {
                workMonthBadgeInline = ' <span style="background: #dc3545; color: white; padding: 2px 6px; border-radius: 3px; font-size: calc(13.5px * var(--ui-scale)); font-weight: normal; white-space: nowrap;">未設定</span>';
                workMonthBadgeBlock = '<div style="margin-top: 4px;"><span style="background: #dc3545; color: white; padding: 2px 6px; border-radius: 3px; font-size: calc(13.5px * var(--ui-scale)); font-weight: normal; white-space: nowrap;">未設定</span></div>';
            } else if (taskWorkMonths.length === 1) {
                const [y, m] = taskWorkMonths[0].split('-');
                workMonthBadgeInline = ` <span style="background: #28a745; color: white; padding: 2px 6px; border-radius: 3px; font-size: calc(13.5px * var(--ui-scale)); font-weight: normal; white-space: nowrap;">${y}年${parseInt(m)}月</span>`;
                workMonthBadgeBlock = `<div style="margin-top: 4px;"><span style="background: #28a745; color: white; padding: 2px 6px; border-radius: 3px; font-size: calc(13.5px * var(--ui-scale)); font-weight: normal; white-space: nowrap;">${y}年${parseInt(m)}月</span></div>`;
            } else {
                const [y1, m1] = taskWorkMonths[0].split('-');
                const [y2, m2] = taskWorkMonths[taskWorkMonths.length - 1].split('-');
                workMonthBadgeInline = ` <span style="background: #28a745; color: white; padding: 2px 6px; border-radius: 3px; font-size: calc(13.5px * var(--ui-scale)); font-weight: normal; white-space: nowrap;">${y1}年${parseInt(m1)}月〜${y2}年${parseInt(m2)}月</span>`;
                workMonthBadgeBlock = `<div style="margin-top: 4px;"><span style="background: #28a745; color: white; padding: 2px 6px; border-radius: 3px; font-size: calc(13.5px * var(--ui-scale)); font-weight: normal; white-space: nowrap;">${y1}年${parseInt(m1)}月</span> <span style="background: #28a745; color: white; padding: 2px 6px; border-radius: 3px; font-size: calc(13.5px * var(--ui-scale)); font-weight: normal; white-space: nowrap;">〜${y2}年${parseInt(m2)}月</span></div>`;
            }
            ...
            if (taskGroup.task.includes(':') || taskGroup.task.includes('：')) {
                ...
                taskDisplayHtml = `${escapeHtml(parts[0])}<span class="task-separator-inline">${separator} ${restPart}</span><span class="task-separator-break"><br><span style="font-size: calc(14px * var(--ui-scale)); font-weight: normal;">${restPart}</span></span>`;
            }
            ...
            sortedProcesses.forEach((proc, index) => {
                ...
                if (est.workMonths.length > 0) {
                    if (est.workMonths.length === 1) {
                        ...
                        processWorkMonthInline = `<span style="background: #28a745; color: white; padding: 2px 6px; border-radius: 3px; font-size: calc(14px * var(--ui-scale)); margin-left: 5px; white-space: nowrap;">${y}年${parseInt(m)}月</span>`;
                        processWorkMonthBlock = `<div style="margin-top: 4px;"><span style="background: #28a745; color: white; padding: 2px 6px; border-radius: 3px; font-size: calc(14px * var(--ui-scale)); white-space: nowrap;">${y}年${parseInt(m)}月</span></div>`;
                    } else {
                        const [y1, m1] = est.workMonths[0].split('-');
                        const [y2, m2] = est.workMonths[est.workMonths.length - 1].split('-');
                        processWorkMonthInline = `<span style="background: #28a745; color: white; padding: 2px 6px; border-radius: 3px; font-size: calc(14px * var(--ui-scale)); margin-left: 5px; white-space: nowrap;">${y1}年${parseInt(m1)}月〜${y2}年${parseInt(m2)}月</span>`;
                        processWorkMonthBlock = `<div style="margin-top: 4px;"><span style="background: #28a745; color: white; padding: 2px 6px; border-radius: 3px; font-size: calc(14px * var(--ui-scale)); white-space: nowrap;">${y1}年${parseInt(m1)}月〜${y2}年${parseInt(m2)}月</span></div>`;
                    }
                } else {
                    processWorkMonthInline = `<span style="background: #dc3545; color: white; padding: 2px 6px; border-radius: 3px; font-size: calc(14px * var(--ui-scale)); margin-left: 5px; white-space: nowrap;">未設定</span>`;
                    processWorkMonthBlock = `<div style="margin-top: 4px;"><span style="background: #dc3545; color: white; padding: 2px 6px; border-radius: 3px; font-size: calc(14px * var(--ui-scale)); white-space: nowrap;">未設定</span></div>`;
                }
    ...
    sortedMatrixVersionKeys.forEach(version => {
        ...
        if (isOtherWorkVersion) {
            ...
            Object.values(versionGroups[version]).forEach(group => {
                ...
                html += `<td style="text-align: center;">
                    ...
                    <div style="font-size: calc(13.5px * var(--ui-scale)); color: #666;">${totalDays.toFixed(1)}人日</div>
                    <div style="font-size: calc(13.5px * var(--ui-scale)); color: #666;">${totalMonths.toFixed(2)}人月</div>
                </td>`;
                html += '</tr>';
            });
        ...
        sortedMatrixTaskGroups.forEach(group => {
            ...
            if (group.task.includes('：')) {
                ...
                taskDisplayHtml = `${escapeHtml(parts[0])}<br><span style="font-size: calc(14px * var(--ui-scale)); font-weight: normal;">${restPart}</span>`;
            }
            ...
            processOrder.forEach(proc => {
                if (group.processes[proc]) {
                    ...
                    html += `<td class="clickable-cell" style="text-align: center; cursor: pointer; transition: background 0.2s; ${bgStyle}"
                        ...
                        <div style="font-size: calc(14px * var(--ui-scale)); color: #666;">(${escapeHtml(p.member)})</div>
                    </td>`;
                } else {
...
export function showTaskDetail(version, task) {
    ...
    if (isClassic) {
        ...
        processOrder.forEach(proc => {
            ...
            if (ests.length > 0) {
                ests.forEach(estimate => {
                    ...
                    html += `</div>`;
                    if (workMonthDisplay) {
                        html += `<div style="font-size: calc(14px * var(--ui-scale)); color: #666; margin-bottom: 6px;">作業月: ${workMonthDisplay}</div>`;
                    }
                    html += `<div style="display: flex; gap: 8px; justify-content: flex-end;">`;
                    html += `<button onclick="editEstimateFromModal(${est.id})" style="padding: 4px 12px; background: var(--info); color: white; border: none; border-radius: 4px; cursor: pointer; font-size: calc(14px * var(--ui-scale));">編集</button>`;
                    html += `<button onclick="deleteEstimateFromTaskModal('${escapedVersion}', '${escapedTask}', ${est.id})" style="padding: 4px 12px; background: #e74c3c; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: calc(14px * var(--ui-scale));">削除</button>`;
                    html += `</div></div>`;
                });
            } else {
                ...
                html += `<button onclick="addProcessFromTaskModal('${escapedVersion}', '${escapedTask}', '${proc}')" style="padding: 4px 12px; background: #198754; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: calc(14px * var(--ui-scale));">追加</button>`;
                html += `</div></div>`;
            }
        });
        ...
        html += `<button onclick="openEditAllProcessesFromTaskModal('${escapedVersion}', '${escapedTask}')" style="padding: 6px 14px; background: #6c757d; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: calc(14px * var(--ui-scale));">全工程を編集</button>`;
        html += `<button onclick="deleteTaskFromModal('${escapedVersion}', '${escapedTask}')" style="padding: 6px 14px; background: #e74c3c; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: calc(14px * var(--ui-scale));">対応を全削除</button>`;
        html += `</div>`;
    } else {
                ...
                html += `
                    <div class="wd-card wd-card-empty">
                        <div class="wd-card-header">
                            <div>
                                <span class="badge badge-${proc.toLowerCase()}">${proc}</span>
                                <span style="color: #999; font-size: calc(14px * var(--ui-scale));">未登録</span>
                            </div>
...
export function showOtherWorkTaskDetail(version, task) {
    ...
    if (isClassic) {
        ...
        taskEstimates.forEach(estimate => {
            ...
            html += `</div>`;
            html += `<div style="font-size: calc(14px * var(--ui-scale)); color: #666; margin-bottom: 8px;">作業月: ${workMonthDisplay}</div>`;
            html += `<div style="display: flex; gap: 8px; justify-content: flex-end;">`;
            html += `<button onclick="editEstimateFromModal(${est.id})"
                        style="padding: 6px 14px; background: var(--info); color: white; border: none; border-radius: 4px; cursor: pointer; font-size: calc(14px * var(--ui-scale)); font-weight: 600;">
                        編集</button>`;
            html += `<button onclick="deleteEstimateFromModal(${est.id})"
                        style="padding: 6px 14px; background: #e74c3c; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: calc(14px * var(--ui-scale)); font-weight: 600;">
                        削除</button>`;
            html += `</div></div>`;
        });
