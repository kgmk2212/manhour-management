    ...
    sortedVersionKeys.forEach(version => {
        ...
        sortedTaskGroups.forEach(taskGroup => {
            ...
            if (taskWorkMonths.length === 0) {
                workMonthBadgeInline = ' <span style="background: #dc3545; color: white; padding: 2px 6px; border-radius: 3px; font-size: calc(11px * var(--ui-scale)); font-weight: normal; white-space: nowrap;">未設定</span>';
                workMonthBadgeBlock = '<div style="margin-top: 4px;"><span style="background: #dc3545; color: white; padding: 2px 6px; border-radius: 3px; font-size: calc(11px * var(--ui-scale)); font-weight: normal; white-space: nowrap;">未設定</span></div>';
            } else if (taskWorkMonths.length === 1) {
                const [y, m] = taskWorkMonths[0].split('-');
                workMonthBadgeInline = ` <span style="background: #28a745; color: white; padding: 2px 6px; border-radius: 3px; font-size: calc(11px * var(--ui-scale)); font-weight: normal; white-space: nowrap;">${y}年${parseInt(m)}月</span>`;
                workMonthBadgeBlock = `<div style="margin-top: 4px;"><span style="background: #28a745; color: white; padding: 2px 6px; border-radius: 3px; font-size: calc(11px * var(--ui-scale)); font-weight: normal; white-space: nowrap;">${y}年${parseInt(m)}月</span></div>`;
            } else {
                const [y1, m1] = taskWorkMonths[0].split('-');
                const [y2, m2] = taskWorkMonths[taskWorkMonths.length - 1].split('-');
                workMonthBadgeInline = ` <span style="background: #28a745; color: white; padding: 2px 6px; border-radius: 3px; font-size: calc(11px * var(--ui-scale)); font-weight: normal; white-space: nowrap;">${y1}年${parseInt(m1)}月〜${y2}年${parseInt(m2)}月</span>`;
                workMonthBadgeBlock = `<div style="margin-top: 4px;"><span style="background: #28a745; color: white; padding: 2px 6px; border-radius: 3px; font-size: calc(11px * var(--ui-scale)); font-weight: normal; white-space: nowrap;">${y1}年${parseInt(m1)}月</span> <span style="background: #28a745; color: white; padding: 2px 6px; border-radius: 3px; font-size: calc(11px * var(--ui-scale)); font-weight: normal; white-space: nowrap;">〜${y2}年${parseInt(m2)}月</span></div>`;
            }
            ...
            sortedProcesses.forEach((proc, index) => {
                ...
                if (est.workMonths.length > 0) {
                    if (est.workMonths.length === 1) {
                        ...
                        processWorkMonthInline = `<span style="background: #28a745; color: white; padding: 2px 6px; border-radius: 3px; font-size: calc(10px * var(--ui-scale)); margin-left: 5px; white-space: nowrap;">${y}年${parseInt(m)}月</span>`;
                        processWorkMonthBlock = `<div style="margin-top: 4px;"><span style="background: #28a745; color: white; padding: 2px 6px; border-radius: 3px; font-size: calc(10px * var(--ui-scale)); white-space: nowrap;">${y}年${parseInt(m)}月</span></div>`;
                    } else {
                        const [y1, m1] = est.workMonths[0].split('-');
                        const [y2, m2] = est.workMonths[est.workMonths.length - 1].split('-');
                        processWorkMonthInline = `<span style="background: #28a745; color: white; padding: 2px 6px; border-radius: 3px; font-size: calc(10px * var(--ui-scale)); margin-left: 5px; white-space: nowrap;">${y1}年${parseInt(m1)}月〜${y2}年${parseInt(m2)}月</span>`;
                        processWorkMonthBlock = `<div style="margin-top: 4px;"><span style="background: #28a745; color: white; padding: 2px 6px; border-radius: 3px; font-size: calc(10px * var(--ui-scale)); white-space: nowrap;">${y1}年${parseInt(m1)}月〜${y2}年${parseInt(m2)}月</span></div>`;
                    }
                } else {
                    processWorkMonthInline = `<span style="background: #dc3545; color: white; padding: 2px 6px; border-radius: 3px; font-size: calc(10px * var(--ui-scale)); margin-left: 5px; white-space: nowrap;">未設定</span>`;
                    processWorkMonthBlock = `<div style="margin-top: 4px;"><span style="background: #dc3545; color: white; padding: 2px 6px; border-radius: 3px; font-size: calc(10px * var(--ui-scale)); white-space: nowrap;">未設定</span></div>`;
                }
        ...
        if (isOtherWorkVersion) {
            ...
            Object.values(versionGroups[version]).forEach(group => {
                ...
                html += `<td style="text-align: center;">
                    ...
                    <div style="font-size: calc(11px * var(--ui-scale)); color: #666;">${totalDays.toFixed(1)}人日</div>
                    <div style="font-size: calc(11px * var(--ui-scale)); color: #666;">${totalMonths.toFixed(2)}人月</div>
                </td>`;
                html += '</tr>';
            });
            ...
            processOrder.forEach(proc => {
                ...
                if (entries && entries.length > 0) {
                    ...
                    const memberLines = entries.map(p =>
                        `<div style="font-size: calc(13px * var(--ui-scale)); color: #666;">(${escapeHtml(p.member)}${entries.length > 1 ? ` ${p.hours.toFixed(1)}h` : ''})</div>`
                    ).join('');
