...
export let layoutDensity = 'comfortable'; // レイアウト密度: 'compact', 'comfortable', 'spacious'
...
// その他工数テンプレート（定期的に発生する作業の自動登録用）
// 各テンプレート: { id: number, name: string, hours: number, applyToAll: boolean }
// applyToAll: true=全メンバー, false=個別指定
export let otherWorkTemplates = [];
...
export function setLayoutDensity(value) {
    layoutDensity = value;
    window.layoutDensity = value;
    // bodyにCSS密度クラスを設定
    document.body.classList.remove('theme-compact', 'theme-comfortable', 'theme-spacious');
    if (value && value !== 'comfortable') {
...
export function setOtherWorkTemplates(value) {
    otherWorkTemplates = value;
    window.otherWorkTemplates = value;
}
...
export let scheduleSettings = {
    ...
    filterHighlightMode: false // ハイライトモード: trueなら非該当をグレーアウト（除外しない）
};
