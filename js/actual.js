...
export function renderMemberCalendar() {
    ...
    if (selectedMonth !== 'all') {
        ...
        html += `<div style="margin-bottom: 15px;">
            ...
            <p style="margin: 0; color: #666; font-size: calc(15px * var(--ui-scale));">稼働日数: ${workedDays}日<span style="margin-left: 20px;">営業日数: ${businessDays}日</span><span style="margin-left: 20px;">合計工数: ${formatHours(totalHours)}h</span></p>
        </div>`;
    }
    ...
    allDates.forEach(date => {
        ...
        const dateDisplay = isHoliday
            ? `${parseInt(m)}/${parseInt(day)} (${dayOfWeek})<span class="holiday-inline"> ${escapeHtml(holiday)}</span><span class="holiday-break"><br><span style="font-size: calc(14.5px * var(--ui-scale)); font-weight: normal;">${escapeHtml(holiday)}</span></span>`
        ...
        if (dayActuals.length === 0) {
            ...
            dayActuals.forEach((actual, index) => {
                ...
                html += `<td style="font-size: calc(15px * var(--ui-scale));">${escapeHtml(actual.version)} - ${escapeHtml(actual.task)} [${escapeHtml(actual.process)}]</td>`;
    ...
    if (isClassic) {
                    ...
                    <div style="margin-bottom: 8px; padding: 10px; background: white; border-radius: 4px;">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div>
                                ...
                                <span style="color: #666; font-size: calc(15px * var(--ui-scale)); margin-left: 10px;">${vacation.hours}h</span>
                            </div>
                            <button onclick="deleteVacationFromModal(${vacation.id}, '${escapeForHandler(member)}', '${escapeForHandler(date)}')" style="background: none; border: none; color: #95a5a6; cursor: pointer; font-size: calc(14.5px * var(--ui-scale)); padding: 4px 8px; text-decoration: underline;">削除</button>
                        </div>
                    </div>
                `;
            });
            ...
            html += `
                <div class="work-item">
                    ...
                    <div style="margin-top: 8px; text-align: right;">
                        <button onclick="editActualFromModal(${actual.id})" style="background: none; border: none; color: var(--info); cursor: pointer; font-size: calc(14.5px * var(--ui-scale)); padding: 4px 8px; text-decoration: underline;">編集</button>
                        <button onclick="deleteActualFromModal(${actual.id}, '${escapeForHandler(member)}', '${escapeForHandler(date)}')" style="background: none; border: none; color: #95a5a6; cursor: pointer; font-size: calc(14.5px * var(--ui-scale)); padding: 4px 8px; text-decoration: underline;">削除</button>
                    </div>
                </div>
            `;
        });
        ...
        html += `
            <div style="margin-top: 20px; padding-top: 15px; border-top: 2px solid #eee; text-align: right;">
                ...
                <span style="color: #666; font-size: calc(15px * var(--ui-scale)); margin-left: 10px;">(${dayActuals.length}件)</span>
            </div>
            <div style="margin-top: 15px; text-align: center; display: flex; gap: 10px; justify-content: center; flex-wrap: wrap;">
                <button onclick="addActualFromCalendar('${escapeForHandler(member)}', '${escapeForHandler(date)}'); closeWorkModal();"
                        style="padding: 10px 20px; background: #4caf50; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: calc(15px * var(--ui-scale)); font-weight: 600;">
                    + 新しい実績を追加
                </button>
                <button onclick="addVacationFromCalendar('${escapeForHandler(member)}', '${escapeForHandler(date)}'); closeWorkModal();"
                        style="padding: 10px 20px; background: #f57c00; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: calc(15px * var(--ui-scale)); font-weight: 600;">
                    + 休暇を追加
                </button>
            </div>
        `;
...
export function renderCalendarGrid() {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (holiday) {
            html += `<div class="calendar-entry" style="color:var(--danger);font-size: calc(14.5px * var(--ui-scale));">${holiday}</div>`;
        }
        if (companyHolidayName && !holiday) {
            html += `<div class="calendar-entry" style="color:var(--warning);font-size: calc(14.5px * var(--ui-scale));">${companyHolidayName}</div>`;
        }
