...
export function renderMemberCalendar() {
    ...
    allDates.forEach(date => {
        ...
        const dateDisplay = isHoliday
            ? `${parseInt(m)}/${parseInt(day)} (${dayOfWeek})<span class="holiday-inline"> ${escapeHtml(holiday)}</span><span class="holiday-break"><br><span style="font-size: calc(13.5px * var(--ui-scale)); font-weight: normal;">${escapeHtml(holiday)}</span></span>`
                ...
                html += `
                    <div style="margin-bottom: 8px; padding: 10px; background: white; border-radius: 4px;">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            ...
                            <button onclick="deleteVacationFromModal(${vacation.id}, '${escapeForHandler(member)}', '${escapeForHandler(date)}')" style="background: none; border: none; color: #95a5a6; cursor: pointer; font-size: calc(14px * var(--ui-scale)); padding: 4px 8px; text-decoration: underline;">削除</button>
                        </div>
                    </div>
                `;
            });
            ...
            html += `
                <div class="work-item">
                    ...
                    <div style="margin-top: 8px; text-align: right;">
                        <button onclick="editActualFromModal(${actual.id})" style="background: none; border: none; color: var(--info); cursor: pointer; font-size: calc(14px * var(--ui-scale)); padding: 4px 8px; text-decoration: underline;">編集</button>
                        <button onclick="deleteActualFromModal(${actual.id}, '${escapeForHandler(member)}', '${escapeForHandler(date)}')" style="background: none; border: none; color: #95a5a6; cursor: pointer; font-size: calc(14px * var(--ui-scale)); padding: 4px 8px; text-decoration: underline;">削除</button>
                    </div>
                </div>
            `;
        });
...
export function renderCalendarGrid() {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (holiday) {
            html += `<div class="calendar-entry" style="color:var(--danger);font-size: calc(14px * var(--ui-scale));">${holiday}</div>`;
        }
        if (companyHolidayName && !holiday) {
            html += `<div class="calendar-entry" style="color:var(--warning);font-size: calc(14px * var(--ui-scale));">${companyHolidayName}</div>`;
        }
