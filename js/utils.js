...
export function showAlert(message, dismissible = false) {
    ...
    if (!isClassic) {
        ...
        if (content) content.classList.remove('alert-closing');

        // タイプに応じたスタイル・アイコン設定
        const type = dismissible ? 'success' : 'error';
        if (content) content.dataset.type = type;
...
export function generateMonthColorLegend(usedMonths, hasMultipleMonths = false, hasUnassigned = false) {
    ...
    let html = '<div style="display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 15px; padding: 10px; background: #f8f9fa; border-radius: 6px; font-size: calc(13px * var(--ui-scale));">';
