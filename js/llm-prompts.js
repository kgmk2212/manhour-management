...

{
  ...
  "outlook": {
    "version_forecasts": [
      {
        "version": "バージョン名",
        "optimistic": "楽観シナリオ: 「残工数100h × 0.9 = 90h」のように必ず具体的な数値を代入すること。『残工数×係数』のままでは不可",
        "realistic": "現実シナリオ: 同様に具体数値を代入（例: 残100h × 1.1 = 110h）",
        "pessimistic": "悲観シナリオ: 同様に具体数値を代入（例: 残100h × 1.3 = 130h）",
...
進行中バージョンごとに楽観・現実・悲観の3シナリオを**実数値で**算出すること。
良い例: 「残工数110h × 1.15 ＝ 127h」
悪い例: 「残工数×0.9＝予想工数」（記号が残っている）

- 楽観: 残工数そのまま（超過なし）
- 現実: 残工数 × 全体平均超過率
- 悲観: 残工数 × 最悪工程の超過率

## 3. 推奨アクション（**必ず3件以上5件以下**、優先度順）
...
**各シナリオは要約データの実数値を代入した計算式を明記すること。**
良い例: 「残工数110h × 1.15 ＝ 127h」（数値と結果が完全に埋まっている）
悪い例: 「残工数×0.9＝予想工数」「残100h×係数」（記号が残っている）

- 楽観: 残工数そのまま（係数 0.9〜1.0）
- 現実: 残工数 × 全体平均超過率（overall.accuracy_percent / 100 を使う）
- 悲観: 残工数 × 最悪工程の超過率（by_process で最も 100% から離れた値）

accuracy_forecastは2〜3文で、工程別の精度傾向を含めること。
resource_risksはメンバー名と具体的な数値を含めること。キャパシティや負荷バランスの問題もあれば言及すること。

## 3. 推奨アクション（必ず3件以上、最大5件、優先度順）

**件数厳守**: recommended_actions の配列は必ず3件以上5件以下で出力すること。2件以下は不可。
見積精度の改善だけでなく、負荷バランス・進捗管理・タスク設計など**異なる観点から**多角的に提案すること。
...
# 制約
- **recommended_actions は必ず3件以上5件以下で、異なる観点から提案すること**
- **version_forecasts の計算式は実数値で埋めること**（例: 「残80h × 1.2 = 96h」、記号を残さない）
- 日本語で回答すること
- JSON以外のテキストを出力に含めないこと`;
