...
- CSP の `connect-src` に `http://localhost:*` と `http://127.0.0.1:*` が含まれており、アプリからローカル Ollama 以外の任意外部 URL には接続できないよう制限されています。設定画面でエンドポイントを変更する場合、CSP で許可されたスキーム/ホストのみ有効です。
