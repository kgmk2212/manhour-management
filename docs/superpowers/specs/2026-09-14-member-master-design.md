...
- 配列の並び順がそのまま表示順（並べ替えは各行の ▲▼ ボタンで1つ隣と入れ替える方式。
  既存の `⠿` ドラッグハンドルはテーブル行・モバイルロングプレス対応込みの専用エンジン
  [`js/estimate.js` `initEstimateDragAndDrop`ほか150行超]で汎用部品ではなく、
  担当者程度の人数・並べ替え頻度に対しては過剰なため転用しない）
...
- **並べ替え**: 各行の ▲（上へ）▼（下へ）ボタンで `members` 配列内の隣接要素と
  入れ替える。先頭行は▲を、末尾行は▼を disabled にする。クリックの都度保存・
  再描画する（ドラッグ&ドロップは実装しない。既存の `⠿` ハンドルはテーブル行・
  モバイルロングプレス対応込みの専用エンジンで汎用部品ではなく、担当者程度の
  人数・並べ替え頻度には過剰なため）
